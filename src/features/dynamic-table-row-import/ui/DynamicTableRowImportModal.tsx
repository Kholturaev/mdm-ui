...
import { useDownloadDynamicTableRowImportTemplate } from '../model/useDownloadDynamicTableRowImportTemplate';
import {
  usePreviewDynamicTableRowImportMutation,
  useExecuteDynamicTableRowImportMutation,
} from '../api/dynamicTableRowImportApi';
...
function StepIndicator({ current }: { current: Step }) {
  ...
  const steps: { id: Step; label: string }[] = [
    { id: 1, label: t('dynamicCharacteristic.rowImport.step1Label') },
    { id: 2, label: t('dynamicCharacteristic.rowImport.step2Label') },
    { id: 3, label: t('dynamicCharacteristic.rowImport.step3Label') },
  ];
...
function ErrorTable({
  ...
  rows: DynamicTableImportPreviewDto['errorRows'];
}) {
  const { t } = useTranslation();
        ...
        <thead>
          <tr className="bg-bg">
            <th className="border-border border-b px-3 py-2 text-left font-semibold">
              {t('dynamicCharacteristic.rowImport.row')}
            </th>
            <th className="border-border border-b px-3 py-2 text-left font-semibold">
              {t('dynamicCharacteristic.rowImport.column')}
            </th>
            <th className="border-border border-b px-3 py-2 text-left font-semibold">
              {t('dynamicCharacteristic.rowImport.error')}
            </th>
          </tr>
        </thead>
        <tbody>
...
export function DynamicTableRowImportModal({
  isOpen,
  onClose,
  tableId,
  tableName,
  ...
  const [preview, setPreview] = useState<DynamicTableImportPreviewDto | null>(
    null,
  );
  const [result, setResult] = useState<DynamicTableImportExecuteDto | null>(
    null,
  );

  const { downloadTemplate, isDownloading } =
    useDownloadDynamicTableRowImportTemplate();
  ...
  const handleFileSelect = (selected: File | null) => {
    ...
    if (!selected.name.toLowerCase().endsWith('.xlsx')) {
      notify.error(t('dynamicCharacteristic.rowImport.onlyXlsx'));
      return;
    }
    setFile(selected);
  };
        ...
        <div className="border-border flex shrink-0 items-center justify-between border-b px-6 py-4">
          <div>
            <div className="text-fg text-base font-semibold">
              {t('dynamicCharacteristic.rowImport.title')}
            </div>
            <div className="text-fg-muted mt-0.5 text-xs">
              {t('dynamicCharacteristic.rowImport.tableLabel', {
                name: tableName,
              })}
            </div>
          </div>
              ...
              <div className="bg-bg border-border rounded-xl border p-5">
                <div className="mb-4">
                  <div className="text-fg text-sm font-semibold">
                    {t('dynamicCharacteristic.rowImport.configTitle')}
                  </div>
                  <div className="text-fg-muted mt-0.5 text-xs leading-relaxed">
                    {t('dynamicCharacteristic.rowImport.templateDesc')}
                  </div>
                </div>
                ...
                >
                  {t('dynamicCharacteristic.rowImport.downloadTemplate')}
                </Button>
              </div>

              <div className="bg-bg border-border rounded-xl border p-5">
                <div className="text-fg mb-3 text-sm font-semibold">
                  {t('dynamicCharacteristic.rowImport.uploadTitle')}
                </div>
                {file ? (
                  <div className="border-border flex items-center gap-3 rounded-lg border bg-white px-4 py-3 dark:bg-white/5">
                    ...
                    <button
                      ...
                      title={t('dynamicCharacteristic.rowImport.removeFile')}
                    >
                      <XCircleIcon size={18} />
                    </button>
                  </div>
                    ...
                    >
                      ...
                      <div>
                        <span className="text-fg text-sm font-medium">
                          {t(
                            'dynamicCharacteristic.rowImport.dropzoneText',
                          )}{' '}
                        </span>
                        <span className="text-primary text-sm font-medium underline-offset-2 hover:underline">
                          {t('dynamicCharacteristic.rowImport.browse')}
                        </span>
                      </div>
                      <div className="text-fg-muted text-xs">
                        {t('dynamicCharacteristic.rowImport.formatHint')}
                      </div>
                    </div>
                  </>
                )}
              ...
              {isPreviewing || !preview ? (
                <div className="flex flex-col items-center justify-center gap-3 py-16">
                  ...
                  <div className="text-fg-muted text-sm">
                    {t('dynamicCharacteristic.rowImport.validating')}
                  </div>
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-3 gap-3">
                    <StatCard
                      label={t('dynamicCharacteristic.rowImport.totalRows')}
                      value={
                        preview.validRows.length + preview.errorRows.length
                      }
                    />
                    <StatCard
                      label={t('dynamicCharacteristic.rowImport.validRows')}
                    ...
                    />
                    <StatCard
                      label={t('dynamicCharacteristic.rowImport.invalidRows')}
                  ...
                  {preview.errorRows.length > 0 && (
                    <div>
                      <div className="text-fg mb-2 text-xs font-semibold">
                        {t('dynamicCharacteristic.rowImport.sampleErrors', {
                  ...
                  {preview.errorRows.length === 0 && (
                    <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-4 py-3 dark:border-green-900/40 dark:bg-green-900/20">
                      ...
                      <span className="text-success text-xs font-medium">
                        {t('dynamicCharacteristic.rowImport.noErrors')}
                      </span>
                    </div>
                  )}
                  ...
                  {preview.validRows.length === 0 && (
                    <div className="bg-bg text-fg-muted rounded-lg px-4 py-3 text-center text-xs">
                      {t('dynamicCharacteristic.rowImport.noValidRows')}
                    </div>
                  )}
                </>
              )}
          ...
          {step === 3 && result && (
            <div className="flex flex-col gap-5">
              <div className="flex flex-col items-center gap-3 py-2 text-center">
                ...
                <div>
                  <div className="text-fg text-base font-semibold">
                    {t('dynamicCharacteristic.rowImport.successTitle')}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <StatCard
                  label={t('dynamicCharacteristic.rowImport.created')}
          ...
          {step === 1 && (
            <>
              ...
              <Button
                ...
                onClick={handleNext}
              >
                {t('dynamicCharacteristic.rowImport.next')}
              </Button>
            </>
          )}
          {step === 2 && (
            <>
              ...
              >
                {t('dynamicCharacteristic.rowImport.execute')}
              </Button>
            </>
          )}
          {step === 3 && (
            <div className="flex w-full justify-end gap-2">
              <Button variant="outline" size="sm" onClick={resetToStart}>
                {t('dynamicCharacteristic.rowImport.newImport')}
              </Button>
